...
def inn_model(img_dims=[1, 28, 28]):
    ...
    conv1 = fr.Node([r1.out0], la.glow_coupling_layer, 
                    {'F_class': fu.F_conv, 'F_args': {'channels_hidden': 128}, 'clamp': 1}, name='conv1')

    conv2 = fr.Node([conv1.out0], la.glow_coupling_layer, 
                    {'F_class': fu.F_conv, 'F_args': {'channels_hidden': 128}, 'clamp': 1}, name='conv2')

    conv3 = fr.Node([conv2.out0], la.glow_coupling_layer, 
                    {'F_class': fu.F_conv, 'F_args': {'channels_hidden': 128}, 'clamp': 1}, name='conv3')
    ...
    fc = fr.Node([r2.out0], la.rev_multiplicative_layer,
                 {'F_class': fu.F_fully_connected, 'clamp': 1}, name='fc')
